...
clear;clc;close all
[s1 fs1] = wavread('D:\Program Files\MATLAB704\work\speaker\train\s1.wav'); %wavread函数是把一个指定的wave文件采样，然后赋给s1，采样率是fs1;
[s2 fs2] = wavread('D:\Program Files\MATLAB704\work\speaker\train\s2.wav'); 
%Question 1； Q1的作用是我认为：把录入的语音采样转换成波形；     
disp('> Question 1');          %disp的作用是显示这个字符串；
t = 0:1/fs1:(length(s1) - 1)/fs1;
figure(1),plot(t, s1), axis([0, (length(s1) - 1)/fs1 -0.4 0.5]);
...
disp('> Question 2: linear spectrum plot');
M = 100;
N = 256;
frames = blockFrames(s1, fs1, M, N);       %调用blockFrames函数，这里有问题？？？？？？？？？？
t = N / 2;
...
%Question 6    %mel倒谱，计算在mel频率变换前后声音文件的频率，是吗？？？？？？？？？？？
